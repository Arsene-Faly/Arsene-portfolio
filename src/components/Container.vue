...
<template>
  ...
  <div class="bg-gradient-to-r from-slate-900 to-slate-700 text-white font-sans">
    <!-- NAVBAR -->
    <nav class="gradient py-4 px-6 shadow-xl fixed top-0 z-50 rounded-3xl w-[98%] max-w-7xl left-1/2 -translate-x-1/2 mt-5">
      <div class="flex justify-between items-center">
        <a href="#accueil" class="logo text-3xl font-extrabold tracking-wide text-green-400 hover:text-green-300 transition">Arsène</a>
        ...
        <div class="hidden md:flex space-x-8">
          <template v-for="link in menuLinks" :key="link.id">
            <a
              ...
              class="font-medium text-lg hover:text-green-400 transition"
            >{{ link.label }}</a>
          </template>
        </div>

        <!-- Bouton toggle mobile -->
        <button @click="isMobileMenuOpen = !isMobileMenuOpen" class="md:hidden text-green-400 focus:outline-none cursor-pointer">
        ...
        <div class="absolute left-1/2 -translate-x-1/2 top-8 w-[92%] max-w-[320px] animate-slide-down">
          <div class="relative flex flex-col items-center space-y-4 bg-slate-800/80 rounded-2xl shadow-xl p-4">
            ...
            <template v-for="link in menuLinks" :key="link.id">
              <a
                :href="`#${link.id}`"
                @click.prevent="handleNavClick(link.id, true)"
    ...
    <footer class="gradientsection rounded-3xl mx-auto mt-10 w-[90%] mb-5 border border-slate-800 shadow-xl">
      <div class="flex flex-col md:flex-row justify-between items-center gap-4 py-6 px-4">
        <div>
          <a href="#accueil" class="text-2xl font-extrabold logo text-green-400 hover:text-green-300 transition">Arsène</a>
        ...
        <div class="flex space-x-4 mt-4 md:mt-0">
          <a href="https://linkedin.com" target="_blank" rel="noopener" class="hover:text-green-400 transition" title="LinkedIn">
            <!-- Icon LinkedIn -->
            <svg class="h-6 w-6" fill="currentColor" viewBox="0 0 24 24"><path d="..."/></svg>
          </a>
          <a href="https://github.com" target="_blank" rel="noopener" class="hover:text-green-400 transition" title="GitHub">
            <!-- Icon GitHub -->
            <svg class="h-6 w-6" fill="currentColor" viewBox="0 0 24 24"><path d="..."/></svg>
          </a>
